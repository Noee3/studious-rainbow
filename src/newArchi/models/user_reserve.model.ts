...
export class UserReserve {
    ...
    static fromDB(userReserveDB: UserReserveDB): UserReserve {
        return new UserReserve(
            userReserveDB.user_address as Address,
            userReserveDB.asset_address as Address,
            userReserveDB.usage_as_collateral_enabled,
            BigInt(userReserveDB.scaled_atoken_balance),
            BigInt(userReserveDB.scaled_variable_debt),
            new Date(Number(userReserveDB.last_updated))
        );
    }
}